...
@Component({
  ...
  styleUrl: './finance-approvals.component.css'
})
export class FinanceApprovalsComponent implements OnInit {
  ...
  load(): void {
    this.service.getPendingFinanceReviews(1, 50).subscribe(result => {
  ...
  approve(item: SponsorshipRequestListItem): void {
    ...
    this.busy = true;
    this.workflowService.financeApprove(item.id, { comments: this.remarks }).subscribe({
      next: () => {
        this.remarks = '';
        this.busy = false;
        this.load();
